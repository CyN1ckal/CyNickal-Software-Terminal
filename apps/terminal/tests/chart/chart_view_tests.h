...
{
    const auto w = myapp::computeVisibleWindow(100, 80.0f, 8.0f, 0, 2);
...
{
    const auto tight = myapp::computeVisibleWindow(100, 80.0f, 8.0f, 0, 2);
    const auto wide = myapp::computeVisibleWindow(100, 80.0f, 16.0f, 0, 2);
...
{
    const auto w = myapp::computeVisibleWindow(100, 80.0f, 8.0f, 20, 2);
    CHECK(w.last == 81);
    CHECK(w.scroll == 20);
}
...
{
    const auto w = myapp::computeVisibleWindow(10, 800.0f, 8.0f, 999, 2);
    CHECK(w.scroll == 0);
    CHECK(w.first == 0);
    CHECK(w.last == 9);
}
...
{
    std::vector<myapp::Bar> bars(3);
    for (myapp::Bar& bar : bars)
    {
        bar.low = 10.0;
        bar.high = 20.0;
    }
    myapp::ChartVisibleWindow win;
    win.first = 0;
    win.last = 2;
    myapp::CChartSettings settings;
    myapp::CChartViewState view;
    const auto y = myapp::computeYLimits(bars, win, settings, view);
...
TEST_CASE("constant range centers on the last visible bar")
{
    ...
    win.last = 1;
    myapp::CChartSettings settings;
    settings.scale_range = myapp::ChartScaleRange::ConstantRange;
